                    ...
                    @foreach ($user_carts as $user_cart)
                        <div class="minicart-item d-flex">
                            ...
                            <div class="product-info">
                                <h2 class="product-title"><a href="#">{{ $user_cart->Product?->name }}</a></h2>
                                <p class="product-vendor">{{ $user_cart->Size?->name }} / {{ $user_cart->Color?->name }}
                                </p>
                                <div class="misc d-flex align-items-end justify-content-between">
                                    <div class="product-remove-area d-flex flex-column align-items-end">
                                        <div class="product-price">&#2547; {{ $user_cart->Product?->discount_price }}</div>
                        ...
                        @php
                            $subTotal = $subTotal + $user_cart->Product?->discount_price;
                        @endphp
                    @endforeach
                </div>
